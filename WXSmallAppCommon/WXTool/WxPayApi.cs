...
namespace WXSmallAppCommon.WXTool
{
    public class WxPayApi
    {
        ...
        */
        ...
        public static WxPayData Transfer(WxPayData inputObj, int timeOut = 6, string appID = WxPayConfig.APPID, string mchID = WxPayConfig.MCHID, string key = WxPayConfig.KEY, string sSLCERT_PASSWORD = WxPayConfig.SSLCERT_PASSWORD, string sSLCERT_PATH = WxPayConfig.SSLCERT_PATH)
        {
            string url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers";
            //检测必填参数
            if (!inputObj.IsSet("amount"))
            {
                throw new WxPayException("商户向用户付款中，amount 为空");
            }
            else if (!inputObj.IsSet("desc"))
            {
                throw new WxPayException("商户向用户付款中，desc 为空");

            }
            else if (!inputObj.IsSet("openid"))
            {
                throw new WxPayException("商户向用户付款中，openid 为空");

            }
            else if (!inputObj.IsSet("partner_trade_no"))
            {
                throw new WxPayException("商户向用户付款中，partner_trade_no 为空");
            ...
            inputObj.SetValue("nonce_str", Guid.NewGuid().ToString().Replace("-", ""));//随机字符串
            inputObj.SetValue("check_name", "NO_CHECK");//检测用户名
            inputObj.SetValue("re_user_name", "");//用户名
